...
// //   id: number;         // Swagger 17페이지 스펙 (contentId) [cite: 665]
// //   content: string;    // 명언 본문 [cite: 666]
// //   imageUrl: string;   // 배경 이미지 URL [cite: 667]
// //   bookTitle: string;  // 책 제목 [cite: 669]
// //   author: string;     // 작가 이름 [cite: 670]
// // }

// // export interface HomeFeedResponse {
// //   date: string;       // 피드 날짜 [cite: 662]
// //   items: HomeSentenceItem[]; // 추천 문장 배열 [cite: 663]
// // }
...
// //     // 💡 [수정 핵심] params에서 'date'를 완전히 삭제했습니다!
// //     // 이렇게 보내야 백엔드가 날짜 조건 없이 DB에 있는 데이터들을 랜덤으로 섞어서 10개씩 뱉어줍니다.
// //     const response = await api.get<ApiResponseHomeFeed>('/sentences/feed', {
// //       params: { 
// //         size // 반환할 문장 개수(10개)만 명시 [cite: 645, 646]
// //       }
// //     });

// //     console.log('🎲 랜덤 홈피드 서버 응답 원본:', JSON.stringify(response.data, null, 2));

// //     if (response.data && response.data.success) {
// //       return response.data.data; 
// //     } else {
// //       throw new Error(response.data?.message || '홈 피드를 가져오지 못했습니다.');
// //     }
// //   },
// // };
// import { api } from './api';
...
// export interface HomeSentenceItem {
//   id: number;         // Swagger 17페이지 스펙 (items 내 id) [cite: 324]
//   content: string;    // 문장 본문 [cite: 325]
//   imageUrl: string;   // 배경 이미지 URL [cite: 326]
//   bookTitle: string;  // 책 제목 [cite: 328]
//   author: string;     // 작가 이름 [cite: 329]
//   createdAt?: string; // 생성일 [cite: 330]
// }

// export interface HomeFeedResponse {
//   date: string;       // 피드 날짜 [cite: 321]
//   items: HomeSentenceItem[]; // 추천 문장 배열 [cite: 322]
// }
...
// // USE_MOCK용 데이터 구조 유지
// const MOCK_HOME_FEED: HomeFeedResponse = {
//   date: '2026-05-19', // [cite: 321]
//   items: [
//     {
//       id: 15, // [cite: 324]
//       content: '책은 마음을 비추는 거울이다.', // [cite: 325]
//       imageUrl: 'https://picsum.photos/400/600',
//       bookTitle: '어린 왕자', // [cite: 328]
//       author: '앙투안 드 생텍쥐페리', // [cite: 329]
//     },
//     {
//       id: 16,
//       content: '사막이 아름다운 건 어딘가에 샘이 숨겨져 있기 때문이야.',
...
//   totalReadingTime: number; // ⏱️ 총 독서 시간 (분 또는 초)
//   totalBooksCount: number;  // 📚 지금까지 완독한 총 권수
// }

// export const homeService = {
//   // 오늘의 홈 피드 데이터 랜덤 조회 (GET /sentences/feed) [cite: 298]
...
//     // Swagger 7페이지 API 명세 연동
//     const response = await api.get<any, ApiResponse<UserStatsResponse>>('/mystats');
//     return response;
//   },
//   getHomeFeed: async (size: number = 10): Promise<HomeFeedResponse> => {
...
export const homeService = {
  ...
  getHomeFeed: async (size: number = 10): Promise<HomeFeedResponse> => {
    ...
    console.log('🌐 [homeService] 랜덤 서버 모드 가동: 무작위 피드를 요청합니다.');
