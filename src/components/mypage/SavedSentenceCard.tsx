...
export default function SavedSentenceCard({
  ...
  // 🎯 [완치 치트키] 백엔드와 프론트 로컬 스토어를 수학적으로 동시 청소하는 듀얼 폭파 핸들러
  const execDeleteSequence = async () => {
    // 1️⃣ [원본 서버 ID 역산 복원] 가공된 ID가 만(10000) 이상이면 원래 서버 고유 ID로 역산합니다!
    const serverId = item.id >= 10000 ? Math.floor(item.id / 10000) : item.id; 

    try {
      console.log(`📡 [실서버 삭제 요청] DELETE /my/sentences/${serverId} 송출 시작...`);
      
      // 2️⃣ 백엔드 진짜 ID를 정밀 타격하여 404 에러 원천 봉쇄!
      const isSuccess = await sentenceService.deleteMySentence(serverId);

      if (isSuccess) {
        // 3️⃣ [쌍둥이 동시 숙청] Zustand 상자(savedSentences)에서 같은 원본 서버 ID를 공유하는 이미지/텍스트 둘 다 한 방에 날려버립니다.
        const currentStored = useSentenceStore.getState().savedSentences;
        const updatedStored = currentStored.filter((s: any) => Math.floor(s.id / 10000) !== serverId && s.id !== serverId);
        
        // Zustand 스토어 강제 새로고침 업데이트로 화면 실시간 동기화
        useSentenceStore.setState({ savedSentences: updatedStored });

        Alert.alert('삭제 완료', '해당 문장의 이미지와 텍스트 버전이 모두 보관함에서 삭제되었습니다. 🗑️');

        // 4️⃣ 상위 탭 리스트 새로고침 콜백 작동
        if (onRefreshList) {
          onRefreshList();
        }
      } else {
        Alert.alert('오류', '서버에서 삭제 처리에 실패했습니다.');
      }
    } catch (error) {
      console.error('❌ 문장 보관함 삭제 작업 최종 실패:', error);
  ...
  return (
    <View style={[styles.card, selectedType === 'text' && styles.textCardContainer]}>
          ...
          <View style={styles.headerLeft}>
            <View style={styles.logoWrapper}>
              <Image
                source={require('../../../assets/logo.png')}
                style={styles.logo}
                contentFit="contain" // 💡 resizeMode 경고 완벽 제거를 위해 contentFit 반영!
              />
            </View>
            <View>
              <Text style={styles.savedTitle}>저장된 문장</Text>
      ...
      {/* 카드 본문 분기 조절 */}
      {selectedType === 'image' ? (
        /* IMAGE 타입 (QuoteCard 감성 밤하늘 시안 적용) */
        <View style={styles.imageCard}>
          <Image
            source={require('../../../assets/logo.png')}
            style={styles.backgroundImage}
            contentFit="cover" // 💡 여기도 contentFit으로 정정해 노란 경고창을 박멸합니다.
          />
          <TouchableOpacity style={styles.bookmark} onPress={execDeleteSequence}>
            <Ionicons name="bookmark" size={24} color="#2563EB" />
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  card: { backgroundColor: '#FFFFFF', borderRadius: 28, marginBottom: 22, overflow: 'hidden' },
  textCardContainer: { borderLeftWidth: 5, borderLeftColor: '#2563EB', borderRadius: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 6, elevation: 2 },
  ...
  backgroundImage: { width: '100%', height: 420 },
  bookmark: { position: 'absolute', top: 14, right: 16 },
  imageSentence: { position: 'absolute', top: '38%', left: 30, right: 30, color: '#111827', fontWeight: '700', lineHeight: 42 },
  textCardContent: { paddingTop: 26, paddingHorizontal: 24, paddingBottom: 16 },
  textSentence: { color: '#1F2937', lineHeight: 30, fontWeight: '500', letterSpacing: -0.3, marginBottom: 20 },
